import { Component, State, Prop } from '@stencil/core';
import { Enseignant } from '../../global/enseignant';
import { MatchResults } from '@stencil/router';


@Component({
  tag: 'enseignant-detail',
  styleUrl: 'enseignant-detail.scss'
})
export class EnseignantDetail {


    @Prop() match: MatchResults;
    @State() enseignant: Enseignant

    componentWillLoad() {
        let noEnseignant = this.match.params.noEnseignant;

        console.log(this.match.params);
        return fetch(
            "https://api-dosispi.cleverapps.io/enseignants/" + noEnseignant
        )
            .then(response => response.json())
            .then(data => {
                this.enseignant = data;
                console.log(noEnseignant);
            });
        }

  render() {
    return(
     //is-bordered is-striped is-narrow is-hoverable is-fullwidth
     <div class="container is-fluid">
     <div class="card">
  <header class="card-header">
  
    <p class="card-header-title">
    No Enseignant : {this.enseignant.noEnseignant}
    </p>
  </header>
  <div class="card-content">
    <div class="content">
    <div class="card-image">
                    <figure class="image">
                      <img class="myimage" src="data:image/png;base64," alt="Image" />
                    </figure>
                  </div>
    </div>
    <div class="content"><br/>
     <p> <i class="fas fa-user"></i>&nbsp;&nbsp;{this.enseignant.nom}&nbsp;{this.enseignant.prenom}</p>
    </div>
    <div class="content">
     <p> <i class="fas fa-map-marker-alt"></i>&nbsp;&nbsp; {this.enseignant.adresse}&nbsp;{this.enseignant.ville}&nbsp;{this.enseignant.codePostal}</p>
    </div>
    <div class="content">
     <p> <i class="fas fa-transgender"></i>&nbsp;&nbsp;&nbsp;{this.enseignant.sexe}</p>
    </div>
    <div class="content">
     <p> <i class="fas fa-at"></i> &nbsp;&nbsp;{this.enseignant.emailPerso}</p>
    </div>
    <div class="content">
     <p><i class="far fa-envelope-open"></i>&nbsp;&nbsp; {this.enseignant.emailUbo}</p>
    </div>
    <div class="content">
     <p> <i class="fas fa-phone-volume"></i>&nbsp;&nbsp;&nbsp;{this.enseignant.telephone}</p>
    </div>
    <div class="content">
     <p> <i class="fas fa-mobile-alt"></i>&nbsp;&nbsp;&nbsp;{this.enseignant.mobile}</p>
    </div>
    <div class="content">
     <p> type : &nbsp;&nbsp;{this.enseignant.type}</p>
    </div>
    <br/>
     &nbsp;&nbsp;                      <stencil-route-link url={'/enseignant/delete/' + this.enseignant.noEnseignant}> <button class="button is-danger" onClick={()=>  confirm('Etes-vous sûr?')}><i class="fas fa-trash-alt"></i>&nbsp;Delete</button></stencil-route-link>

<br/>
  </div>
  <footer class="card-footer">

  </footer>
  
</div>
<br/>
  
 
</div>
      
);
}}

