import { Component, State } from '@stencil/core';

@Component({
  tag: 'enseignant-search',
  styleUrl: 'enseignant-search.scss'
})

export class EnseignantSearchByName {

  @State() enseignants: any = "";

  InputEmail: HTMLInputElement;

  findEnsByEmail() {

    let emailUbo = this.InputEmail.value;
    return fetch('http://api-dosispi.cleverapps.io/enseignants/emailUbo/' + emailUbo)
      .then(response => response.json())
      .then(data => {
        this.enseignants = data;
      });
  }

  render() {

    if (this.enseignants == "") {
      return (

        <div class="container is-fluid">
          <br />
          <br />
          <div class="box">
            <div class="field is-grouped">
              <p class="control is-expanded">
                <input class="input is-medium is primary is rounded" ref={(el: HTMLInputElement) => this.InputEmail = el} type="text" placeholder="email de l'enseignant" />
              </p>
              <p class="control">
                <button class="button is-info is-medium" onClick={() => this.findEnsByEmail()}>Search</button>
              </p>
            </div>
          </div>
          <br />
          <br /> <br/>
          <br/>
          <br/>
          <br/>
          <br/>
          <br />
          <br />

          <br/>
         



        </div>
      )
    }
    else {
      return (
        <div class="container is-fluid">
          <div class="center">

            <div class="card">
              <header class="card-header">

                <p class="card-header-title">
                  No Enseignant : {this.enseignants.noEnseignant}
                </p>
              </header>
              <div class="card-content">
                <div class="content">
                  <div class="card-image">
                    <figure class="image">
                      <img class="myimage" src="data:image/png;base64," alt="Image" />
                    </figure>
                  </div>
                </div>
                <div class="content"><br />
                  <p> <i class="fas fa-user"></i>&nbsp;&nbsp;{this.enseignants.nom}&nbsp;{this.enseignants.prenom}</p>
                </div>
                <div class="content">
                  <p> <i class="fas fa-map-marker-alt"></i>&nbsp;&nbsp; {this.enseignants.adresse}&nbsp;{this.enseignants.ville}&nbsp;{this.enseignants.codePostal}</p>
                </div>
                <div class="content">
                  <p> <i class="fas fa-transgender"></i>&nbsp;&nbsp;&nbsp;{this.enseignants.sexe}</p>
                </div>
                <div class="content">
                  <p> <i class="fas fa-at"></i> &nbsp;&nbsp;{this.enseignants.emailPerso}</p>
                </div>
                <div class="content">
                  <p><i class="far fa-envelope-open"></i>&nbsp;&nbsp; {this.enseignants.emailUbo}</p>
                </div>
                <div class="content">
                  <p> <i class="fas fa-phone-volume"></i>&nbsp;&nbsp;&nbsp;{this.enseignants.telephone}</p>
                </div>
                <div class="content">
                  <p> <i class="fas fa-mobile-alt"></i>&nbsp;&nbsp;&nbsp;{this.enseignants.mobile}</p>
                </div>
                <div class="content">
                  <p> type : &nbsp;&nbsp;{this.enseignants.type}</p>
                </div>
              </div>
              

            </div>
          </div>

          <br />

        </div>  )}
      }
}